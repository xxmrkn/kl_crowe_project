...
def visualize_total_image(path,id,labels1,labels2,num,normal_acc,neighbor_acc,flag):
    ...
    for x,(image_path,id,label1,label2) in enumerate(zip(path,id,labels1,labels2)):
        ...
        plt.axis("off")
    if flag==1:
        new = [[0]*3 for i in range(len(path))]

        for i in range(len(path)):
            new[i][0],new[i][1],new[i][2] = id[i][:-4],labels1[i],labels2[i]

        print(new)

        with open(f"outputs/{CFG.model_name}/{CFG.n_fold}fold/path_{CFG.model_name}_{CFG.n_fold}fold_{CFG.epochs}epoch_outlier.csv", 'w', newline="") as f:
            writer = csv.writer(f)
        ...
        new2 = [[0]*3 for i in range(len(path))]

        for i in range(len(path)):
            new2[i][0],new2[i][1],new2[i][2] = id[i][:-4],labels1[i],labels2[i]

        print(new)

        with open(f"outputs/{CFG.model_name}/{CFG.n_fold}fold/path_{CFG.model_name}_{CFG.n_fold}fold_{CFG.epochs}epoch_outlier2.csv", 'w', newline="") as f:
            writer = csv.writer(f)
            writer.writerows(new)
